...
public function sendMpesa()
{
    ...
    if ($authentication->successful()) {
        ...
        $password = base64_encode(config('mpesa.short_code') . config('mpesa.pass_key') . $timestamp);
        ...
        $data = [
            ...
            "PartyB" => config('mpesa.till_no'),
            "PhoneNumber" => auth()->user()->phone_number,
            "CallBackURL" => url('api/stk_callback'),
            "AccountReference" => config('mpesa.till_no'),
...
private function makeApiCallToCheckTransactionStatus($transactionReference)
{
    ...
    if ($authentication->successful()) {
        ...
        $response = Http::withToken($token)->post('https://api.safaricom.co.ke/mpesa/stkpushquery/v1/query', [
