...
public function sendMpesa()
{
    try {
        // Get Payment Gateway
        $paymentGateway = PaymentGateways::whereName('Mpesa')->firstOrFail();
        ...
        // Calculate taxes
        $taxes = $this->settings->tax_on_wallet ? ($amount * auth()->user()->isTaxable()->sum('percentage') / 100) : 0;
        $taxesPayable = $this->settings->tax_on_wallet ? auth()->user()->taxesPayable() : null;

        // Calculate total amount
        $amountFixed = number_format($amount + ($amount * $fee / 100) + $cents + $taxes, 2, '.', '');

        // URLs for success and cancel
        $urlSuccess = url('my/wallet');
        $urlCancel = url('my/wallet');

        // Authenticate with Mpesa
        $authentication = Http::withBasicAuth(env('MPESA_CONSUMER_KEY'), env('MPESA_CONSUMER_SECRET'))
        ...
        if ($authentication->successful()) {
            ...
            if ($stkpush->successful() && $stkpush['ResponseCode'] == 0) {
                ...
                \Log::error('Mpesa STK Push Error: ' . $stkpush->body());
                return response()->json([
                    'success' => false,
                    'status' => 'cancelled',
                    'status_info' => __('general.cancelled_deposit'),
                    'redirect_url' => $urlCancel
                ]);
            }
        } else {
            \Log::error('Mpesa Authentication Error: ' . $authentication->body());
            return response()->json([
                'success' => false,
                'status' => 'error',
                'status_info' => __('general.error_occurred'),
                'redirect_url' => $urlCancel
            ]);
        }
    } catch (\Exception $e) {
    ...
    if (isset($callback['Body']['stkCallback']['MerchantRequestID'])) {
        ...
        if ($transaction) {
            if ($callback['Body']['stkCallback']['ResultCode'] == 0) {
                ...
                // Add Funds to User
                User::find($transaction->user_id)->increment('wallet', $transaction->amount);

                // Notify Admin via Email
                try {
                    Notification::route('mail', $this->settings->email_admin)
                        ->notify(new AdminDepositPending($transaction));
                } catch (\Exception $e) {
                    \Log::info($e->getMessage());
                }
