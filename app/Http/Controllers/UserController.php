...
class UserController extends Controller
{
  ...
  public function formatPhoneNumber($phoneNumber)
    {
        // Check if the phone number starts with '07' or '011'
        if (strlen($phoneNumber) >= 10 && ($phoneNumber[0] == '0')) {
            // For numbers starting with '07', replace '07' with '2547'
            if (substr($phoneNumber, 0, 2) == '07') {
                $phoneNumber = '2547' . substr($phoneNumber, 2);
            }
            // For numbers starting with '011', replace '011' with '25411'
            elseif (substr($phoneNumber, 0, 3) == '011') {
                $phoneNumber = '25411' . substr($phoneNumber, 3);
            }
        }
        return $phoneNumber;
    }
    ...
    public function updateSettings()
    {
        ...
        $validator = Validator::make($input, [
            ...
            'phone_number'  => 'required|min:10|max:12|string',  // Adjusted to match the length of Kenyan phone numbers
        ...
        $input['phone_number'] = $this->formatPhoneNumber($input['phone_number']);

        // Debug: Check the formatted phone number before saving
        \Log::info('Formatted Phone Number: ' . $input['phone_number']);

        $user = User::find($id);
        $user->profession = trim(strip_tags($input['profession']));
        $user->phone_number = trim(strip_tags($input['phone_number']));
  ...
  public function updateSettingsPage()
  {
    ...
    $validator = Validator::make($input, [
      ...
      'phone_number'  => 'required|min:2|max:20|string',
