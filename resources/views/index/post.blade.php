            ...
            <div class="@if ($blogs->count() == 0) col-md-12 @else col-md-8 @endif py-5">
                <div class="row no-gutters border rounded flex-md-row mb-4 shadow-sm h-md-250 position-relative">
                    ...
                    <div class="col p-4 d-flex flex-column position-static">
                        ...
                        @if ($response->media_type == 'reel')
                            <div class="mt-4">
                                <video src="{{ Helper::getFile(config('path.videos') . $response->video) }}" controls class="w-100"></video>
                                <div class="d-flex flex-column align-items-center mt-3">
                                    <a href="#" class="btn btn-sm btn-primary mb-2">{{ __('general.tip') }}</a>
                                    <a href="#" class="btn btn-sm btn-secondary mb-2">{{ __('general.share') }}</a>
                                    <a href="#" class="btn btn-sm btn-success">{{ __('general.comment') }}</a>
                                </div>
                            </div>
                        @endif
            ...
            @if ($blogs->count() != 0)
                <div class="col-md-4 mb-4 py-lg-5">
                    ...
                    @foreach ($blogs as $blog)
                        <a href="{{ url('blog/post', $blog->id) . '/' . $blog->slug }}">
                            <div class="w-100 d-block" style="background: @if ($blog->image != '') url({{ Helper::getFile(config('path.admin') . $blog->image) }}) @endif #505050 center center; border-radius: 6px; background-size: cover;">
                                    ...
                                    <div class="media">
                                        <div class="media-body">
                                            <h5 class="media-heading mb-1">
                                                <a href="{{ url('blog/post', $blog->id) . '/' . $blog->slug }}" class="stretched-link text-white">
                                                    <strong>{{ $blog->title }}</strong>
                                                </a>
                                                <small class="text-white w-100 d-block mb-2">{{ '@' . $blog->user()->name }} - {{ Helper::formatDate($blog->date) }}</small>
                                                <p class="text-white font-weight-light">{{ Str::limit(strip_tags($blog->content), 60, '...') }}</p>
                                            </h5>
                                        </div>
                                    </div>
                                </li>
