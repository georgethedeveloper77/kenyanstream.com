    ...
    <div class="card mb-4 card-border-0 rounded-large shadow-large">
        ...
        <div class="card-footer bg-white border-0 pt-0 rounded-large">
            <div class="justify-content-between align-items-center">
                ...
                <!-- Reels Upload -->
                <input type="file" name="reel" id="fileReel" accept="video/mp4,video/x-m4v,video/quicktime" class="visibility-hidden">
                <button type="button" class="btn btn-post btn-tooltip-form e-none @if (auth()->user()->dark_mode == 'off') text-primary @else text-white @endif rounded-pill" data-toggle="tooltip" data-placement="top" title="{{__('general.upload_reel')}}" onclick="$('#fileReel').trigger('click')" disabled>
                    <i class="feather icon-film f-size-20 align-bottom"></i>
                </button>

                <div class="w-100">
                    <span id="previewReel"></span>
                    <a href="javascript:void(0)" id="removeReel" class="text-danger p-1 px-2 display-none btn-tooltip-form" data-toggle="tooltip" data-placement="top" title="{{__('general.delete')}}"><i class="fa fa-times-circle"></i></a>
                </div>
...
<script>
    // Reel preview
    $('#fileReel').on('change', function() {
        var file = this.files[0];
        var reader = new FileReader();
        reader.onload = function(e) {
            $('#previewReel').html('<video src="' + e.target.result + '" controls class="w-100 mt-3"></video>');
            $('#removeReel').removeClass('display-none');
        };
        reader.readAsDataURL(file);
    });

    // Remove reel
    $('#removeReel').on('click', function() {
        $('#fileReel').val('');
        $('#previewReel').html('');
